...
class Common_model extends CI_Model {
	...
	function getDateActivities($today,$tomorrow,$id1) {
		...
		function userreport($startdate,$endate,$user)
		{
			...
			$endate=date('Y-m-d H:i:s', strtotime($endate)+86400);
			
			$q=$this->db->select('*')->from('activities')->where('user_id',$user)->where('activity_date >=', $startdate)->where('activity_date <=',$endate)->group_by('activity_date')->order_by('id','desc')->get();
			$r=$q->result();	
			//echo $this->db->last_query();
			//var_dump($r);
			//die;
		...
		function sendmessage($message,$text,$aid)
		{ 
			
			$serialized = json_encode($message);
			$mes = json_decode($serialized);
			...
			foreach($mes as $mm)
			{
				...
				foreach ($mm as $m) {
				 ...
				 $str = substr($m, 1);
				 $s=substr($str,0,4);
				 $q=$this->db->select('*')->from('users')->like('firstname',$s)->get();
				$r=$q->row();
				
					$for=$r->id;
					
				$newdata['for_id']=$for;
				$taggeddata['tagin']=$for;
				$newdata['from_id']=$_SESSION['id'];
				$taggeddata['tagged_by']=$_SESSION['id'];
				$newdata['is_read']=0;
				$temp="<span class='notify'>".$_SESSION['firstname']."</span>";
				$tempA="<span class='actext'>".substr($text,0,50)."</span>";
				$newdata['text']=$temp." tagged you in ".$tempA;
				$newdata['ac_id']=$aid;
/*for tag*/		$taggeddata['ac_id']=$aid;
				$this->db->insert('notification',$newdata);
				$this->db->insert('tagged',$taggeddata);
		...
		function checkemail($email)
			{
				$q=$this->db->select('*')->from('users')->where('email',$email)->get();
				$r=$q->row();
				return $r;
			}
			//new function
			...
			{
				$id=$_SESSION['id'];
				$q=$this->db->select('*')->from('tagged')->where('tagin',$id)->order_by('id','desc')->limit(15,0)->get();
				$r=$q->result();
				
				foreach($r as $r1) {
				$qq=$this->db->select('activity_text')->from('activities')->where('id',$r1->ac_id)->get();
				$temp=$qq->row();
				//print_r($temp);die;
				$r1->text=$temp->activity_text;
				}
				foreach($r as $r1) {
				$qq=$this->db->select('firstname')->from('users')->where('id',$r1->tagged_by)->get();
				$temp=$qq->row();
				$r1->firstname=$temp->firstname;
				}
				return $r;
			}
			function getmymentions($tagged_by,$tagin)
			{
				
				$q=$this->db->select('*')->from('tagged')->where('tagin',$tagin)->where('tagged_by',$tagged_by)->order_by('id','desc')->limit(15,0)->get();
				$r=$q->result();
