  ...
  <body>
      ...
      <div class="large-6 columns activity-list">
      	...
      	<ul class="activities">
        	...
        	//die();
        	//echo $var;
        	//print_r($aa2[0]->firstname);
        	//print_r($use);
        	//die;
			if(count($activities)==0) :
				?>
                	<li class="no-activity" id="noact">No activities were added by <? echo ucwords($user[0]->firstname);echo " ";?>on <?if(isset($date)) {echo" ";echo date('d-m-Y', strtotime($date));} else {echo " ";echo date('d-m-Y', time());} ?>
                	</li>
                <?
			else :
				?><li class="no-activity" id="noact">Activities added by <? echo ucwords($user[0]->firstname);?> on <? $y=strtotime($activities[0]->activity_date); echo date('d-m-Y', $y)?>
             <div class="activity-datepicker">
        	<? echo ucwords($user[0]->firstname);?>`s activities by date 
        	</li>
				<?
				//print_r($activities);
				foreach($activities as $activity) {
					...
					<li>
					 <ul class="actions">
						...
						if($activity->is_approved):
							...
							{
							?>
							<li class="approved "><span >Approved</span><a href='<? echo site_url(); ?>/home/unapprove/<?echo $activity->id; echo "/".$activity->user_id?>' class="link"><i data-tooltip class="fa fa-check-circle tip-right" title="Marked as done!"></i></a></li>
							<?}
							
							else
							{?>
							<li class="approved"><span >Approved</span><i data-tooltip class="fa fa-check-circle tip-right" title="Marked as done!"></i></li>				
							<?
							}
						else:
							if($_SESSION['is_admin']==1)
							{?>
							<li class="unapproved"><span>Unapproved</span><a href='<? echo site_url(); ?>/home/approve/<?echo $activity->id; echo "/".$activity->user_id?>' class="link "> <i class="fa fa-check-circle"></i><span data-tooltip title="Marked as done!"></span></a></li>
							<?
							}
							else
							{
					...
					<span class="activity-time"><? $x=strtotime($activity->activity_date); echo date('d-m-Y h:i a', $x) ?></span>
...
$(document).ready(function() {
  ...
  .on('changeDate', function(ev){
   ...
   window.location.replace("<? echo site_url(); ?>/home/getDateActivities/<? echo $var;?>/"+aaa);
});
  </script>
